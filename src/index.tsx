...
const EasyCrop = forwardRef((props: EasyCropProps, ref) => {
    const {
        src,
        // @ts-ignore
        aspect, shape, grid, hasZoom, zoomVal, rotateVal, setZoomVal, setRotateVal, minZoom, maxZoom,
        onComplete,
...
export const ZegImgCrop = forwardRef((props: ZegImgCropProps, ref: ForwardedRef<UploadProps>) => {
                ...
                >
                    ...
                    {hasZoom && (
                        <div className={`${pkg}-control zoom`}>
                            ...
                            <Slider className={`${pkg}-control-slider`}
                                    min={minZoom}
                                    max={maxZoom}
                                    step={ZOOM_STEP}
                                    value={zoomVal}
    ...
    return (
        // @ts-ignore
        <LocaleReceiver>{(locale, localeCode) => renderComponent(localeCode === 'zh-cn' ? '编辑图片' : 'Edit image')}</LocaleReceiver>
    );
});
